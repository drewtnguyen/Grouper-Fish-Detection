...
if [ "$ext" = "MTS" ] || [ "$ext" = "mts" ] ; then
    ...
    ffmpeg -hide_banner -noautorotate -loglevel panic -i "${fn}" -vf "fps=1/${N},yadif=0:-1:1" -q:v 1 util/temp_do_not_store_files_here/gpu_"${G}"/outfile_%d.jpg
    ...
    ffmpeg -hide_banner -loglevel panic -i "${fn}" -vf fps=1/${N} -q:v 1 "util/temp_do_not_store_files_here/gpu_${G}/outfile_%d.jpg"
fi 
