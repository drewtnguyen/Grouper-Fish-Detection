...
if [ ! $# -eq 10 ]
  then
    echo "Error; the format is process_one_video.sh --input_file FILE --output_dir DIRECTORY --period N --batch_size B --cuda_visible G"
    exit 2
fi
...
mkdir -p util/temp_do_not_store_files_here/gpu_"${G}"

if [ "$ext" = "MTS" ] || [ "$ext" = "mts" ] ; then
    echo "Deinterlacing MTS file..." 
    ffmpeg -y -hide_banner -noautorotate -loglevel panic -i "${fn}" -vf "fps=1/${N},yadif=0:-1:1" -q:v 1 util/temp_do_not_store_files_here/gpu_"${G}"/outfile_%d.jpg
    ...
    ffmpeg -y -hide_banner -loglevel panic -i "${fn}" -vf fps=1/${N} -q:v 1 "util/temp_do_not_store_files_here/gpu_${G}/outfile_%d.jpg"
fi 

echo "stills generated. now renaming stills..."

#now process the stills we've made
python util/process_stills.py --N "${N}" --vidname "${fn}" --to_process "util/temp_do_not_store_files_here/gpu_${G}" --outdir "${temp_dir}"
...
ffmpeg -y -hide_banner -loglevel panic -framerate 1/${N} -i "${vid_dir}/imgs_to_stitch/img_%d.jpg" -pix_fmt yuv420p -c:v libx264 "${vid_dir}/${fn_no_ext}_tracked.mp4" #stitches images to make annotated video
...
rm -rf "${temp_dir}"
rm "$dat_no_id"
rm -rf "util/temp_do_not_store_files_here/gpu_${G}"/*
